...
SELECT prov, distrito FROM ubigeo WHERE dpto = 'ica';
SELECT prov, distrito FROM ubigeo WHERE prov = 'Chincha';


DELIMITER $$
CREATE PROCEDURE spu_listar_departamento()
BEGIN
	SELECT DISTINCT dpto FROM ubigeo;
END $$

DELIMITER $$
CREATE PROCEDURE spu_listar_provincias(IN _dpto VARCHAR(32))
BEGIN
	SELECT DISTINCT prov FROM ubigeo WHERE dpto = _dpto;
END $$

DELIMITER $$
CREATE PROCEDURE spu_listar_distrito(IN _prov VARCHAR(32))
BEGIN
